...
This exercise is part of the **Intermediate Checkpoints** series. By completing it, you'll master the logic needed to solve this type of problem and be prepared for similar challenges.

**Challenge:** Write a function that takes three strings as arguments: `text`, `old`, and `new`. The function should return a new string where all occurrences of `old` in `text` are replaced with `new`.
