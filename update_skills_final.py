...
Uses 01-only1/skills.md as a template for structure (but does NOT modify it).
Uses LEARNING_PATH.md for correct ordering.
"""
...
PACKAGE_REFERENCES = {
    ...
    "encoding/json": "[encoding/json package](https://pkg.go.dev/encoding/json)",
    "net/http": "[net/http package](https://pkg.go.dev/net/http)",
    "piscine": "[piscine package](https://github.com/01-edu/piscine-go)",
}

def get_learning_path_order():
    """Extract the correct order from LEARNING_PATH.md"""
    learning_path = BASE_DIR / "LEARNING_PATH.md"
    content = learning_path.read_text(encoding="utf-8")
    
    # Find all backtick-quoted directory names in order
    pattern = r'`(\d{2,3}-[^`]+)`'
    matches = re.findall(pattern, content)
    return matches

def get_dirs():
    """Get all directories that have both README.md and skills.md, in learning path order"""
    ordered_names = get_learning_path_order()
    dirs = []
    
    for name in ordered_names:
        d = BASE_DIR / name
        if d.is_dir() and (d / "README.md").exists() and (d / "skills.md").exists():
            if name != "01-only1":  # Skip the template
                dirs.append(d)
    
    # Also add any remaining dirs not in learning path (excluding 01-only1)
    for d in sorted(BASE_DIR.iterdir()):
        if d.is_dir() and (d / "README.md").exists() and (d / "skills.md").exists():
            if d.name != "01-only1" and d not in dirs:
                dirs.append(d)
...
def extract_challenge_info(readme_content, dir_name):
    """Extract challenge name and description from README"""
    lines = readme_content.strip().split("\n")
    # Clean up dir name for challenge name (remove number prefix)
    challenge_name = re.sub(r'^\d+-', '', dir_name).replace("-", " ").title()
    instructions = []
    
    for line in lines:
        line = line.strip()
        # Look for title
        if line.startswith("## ") and challenge_name == dir_name.replace("-", " ").title():
            challenge_name = line.replace("## ", "").strip()
        elif line.startswith("# ") and challenge_name == dir_name.replace("-", " ").title():
            challenge_name = line.replace("# ", "").strip()
        # Collect instruction lines
            ...
            if not line.startswith("//") and not line.startswith("package") and not line.startswith("import") and not line.startswith("func main") and not line.startswith("fmt.") and not line.startswith("piscine."):
                instructions.append(line)
    
    # Get a good description from first few instruction lines
    desc = " ".join(instructions[:3]) if instructions else challenge_name
    ...
    return challenge_name, desc

def get_prev_next_links(dir_name, all_dirs):
    """Get previous and next directory links based on learning path order"""
    # Build ordered list including 01-only1 at the start
    ordered_names = get_learning_path_order()
    all_ordered = []
    for name in ordered_names:
        d = BASE_DIR / name
    ...
    try:
        idx = all_ordered.index(dir_name)
    except ValueError:
        return "", ""
    ...
    if idx > 0:
        prev_name = all_ordered[idx - 1]
        prev_link = f"**Previous:** [{prev_name}](../{prev_name}/skills.md)"
    
    if idx < len(all_ordered) - 1:
        next_name = all_ordered[idx + 1]
...
def detect_concepts(readme_content, dir_name):
    ...
    # String operations
    if any(word in content_lower for word in ["string", "str", "text", "word", "char", "rune"]):
    ...
    # Conditionals
    if any(word in content_lower for word in ["if", "condition", "return", "bool", "true", "false"]):
        concepts.append("Conditional logic and boolean returns")
    
    # Slices
    if "slice" in content_lower or "[]int" in readme_content or "[]string" in readme_content:
    ...
    # Numbers/Math
    if any(word in content_lower for word in ["int", "uint", "number", "math", "prime", "digit", "numeric"]):
        concepts.append("Numeric operations and type conversion")
    
    # Arrays
    if "[10]" in readme_content or "[10]byte" in readme_content:
...
def get_relevant_packages(readme_content):
    ...
    pkg_map = {
        ...
        "strings": ["strings.", "split", "join", "replace", "trim", "hasprefix", "hassuffix", "index", "contains", "builder"],
...
def generate_skills_content(dir_name, readme_content, prev_link, next_link):
    ...
    challenge_name, instructions = extract_challenge_info(readme_content, dir_name)
...
def main():
    dirs = get_dirs()
    print(f"Found {len(dirs)} directories with README.md and skills.md (excluding 01-only1)")
