...
**Challenge:** Write a function `FifthAndSkip()` that takes a `string` and returns another `string`. The function separates every five characters of the `string` with a space and removes the sixth one.

## New Concept

Periodic extraction (every 5th) with modular arithmetic
