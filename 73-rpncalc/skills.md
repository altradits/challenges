...
**Challenge:** Write a program that takes a `string` which contains an equation written in `Reverse Polish Notation` (RPN) as its first argument, that evaluates the equation, and that prints the result on the standard output followed by a newline (`'\n'`).

## New Concept

Reverse Polish notation with stack-based calculation
